...
class WCSPHSolver(SPHBase):
    ...
    def compute_non_pressure_forces_task(self, p_i, p_j, ret: ti.template()):
        ...
        if self.ps.material[p_j] == self.ps.material_fluid:
            # Fluid neighbors
            diameter2 = self.ps.particleDiameter * self.ps.particleDiameter
            x_j = self.ps.x[p_j]
            r = x_i - x_j
            r2 = r.dot(r)
            if r2 > diameter2:
                ...
                ret -= self.surface_tension / self.ps.m[p_i] * self.ps.m[p_j] * r * self.cubic_kernel(ti.Vector([self.ps.particleDiameter, 0.0, 0.0]).norm())
    ...
    def compute_pressure_forces(self):
        ...
        for p_i in ti.grouped(self.ps.x):
            if self.ps.is_static_rigid_body(p_i):
                ...
                continue
            dv = ti.Vector([0.0 for _ in range(3)])
