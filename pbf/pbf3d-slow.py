...
def render(scene):
    scene.particles(positions, color = (0, 1, 1), radius = particle_radius_in_world)
...
def init_particles():
    ...
    for i in range(num_particles):
        _square = i % (num_particles_x * num_particles_z)
        _height = i // (num_particles_x * num_particles_z)
        delta = h_ * 0.8
        #offs = ti.Vector([(boundary[0] - delta * num_particles_x) * 0.5,
        #                  boundary[1] * 0.02, 0.0])
        offs = ti.Vector([0, 0, 0])
        positions[i] = ti.Vector([_square % num_particles_x, _height, _square // num_particles_z]) * delta + offs
        for c in ti.static(range(dim)):
            velocities[i][c] = (ti.random() - 0.5) * 4

    board_states[None] = ti.Vector([boundary[0] - epsilon, -0.0, boundary[2] - epsilon])


def main():
    init_particles()
    
    window = ti.ui.Window("simple pendulum", (800, 800))
