...
p_matrix = ti.field(ti.f32, shape=(resolutionX * resolutionX, resolutionX * resolutionX))
dt = 0.02
dx = 1.0
inv_dx = 1.0 / dx
...
_diff_pressures = ti.field(dtype=ti.f32, shape=(resolutionX, resolutionX))
_dye_buffer = ti.field(dtype=ti.f32, shape=(resolutionX, resolutionX))
_new_dye_buffer = ti.field(dtype=ti.f32, shape=(resolutionX, resolutionX))
...
def advect(vf: ti.template(), qf: ti.template(), new_qf: ti.template()):
    ...
    for IX, IY in vf:
        # Backtrace:
        coord_curr = ti.Vector.field([IX, IY]) + ti.Vector.field([0.5, 0.5])
        vel_curr = vf[IX, IY]
...
def correct_divergence(vf: ti.template(), vf_new: ti.template(), pf: ti.template()):
    for i, j in vf:
        vf_new[i, j] = vf[i, j] - ti.Vector.field([(pf[i + 1, j] - pf[i - 1, j]) / 2.0, (pf[i, j + 1] - pf[i, j - 1]) / 2.0])
